...
class Car(pygame.sprite.Sprite):  # Car є нащадком базового класу Sprite, який написали розробники PyGame.
    ...
    def move(self):
        ...
        if self.velocity > 0:
            ...
            if self.velocity < 0:
                self.velocity = 0  # Зупинка, аби машина не почала сама їхати назад
    ...
    def rotate(self):
        self.image = pygame.transform.rotate(self.original_image, self.angle)  # Поворот зображення через поворот самої машини
        self.rect = self.image.get_rect(center=self.rect.center)
        self.mask = pygame.mask.from_surface(self.image)

    def update(self):  # Це метод викликається командою self.all_sprites.update() у файлі game.py
        self.get_input()  # Зчитування кнопок
        self.move()  # Попередньо описаний метод
        self.rotate()  # Попередньо описаний метод
